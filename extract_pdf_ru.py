...
DEFAULT_PROMPT = (
    "Extract cyrilic text please"
    "It is actually azerbaijani language, written in cyrilic script. Some letters are different from russian. Extract what u see, its latin and letter ə is present."
    "Return ONLY the extracted text (no explanations). "
    "Preserve meaning, formatting, headings, and line breaks.\n"
...
def convert_to_latin_azerbaijani(client: OpenAI, model: str, text: str, max_tokens: int = 2000):
    prompt = (
        "Convert the following text from Azerbaijani written in Cyrillic "
        "into proper modern Azerbaijani in Latin script.\n\n"
        "Rules:\n"
        "- This is Azerbaijani, not Russian.\n"
        "- Use correct Azerbaijani letters: ə, ı, ö, ü, ğ, ş, ç.\n"
        "- Preserve meaning, formatting, headings, and line breaks.\n"
...
def process_pdf(
    ...
    for i in page_iter:
        ...
        page_header = f"===== {pdf_path.name} | Page {i+1}/{total_pages} =====\n"
        combined_cyr.append(page_header + text + "\n")

        latin_text = convert_to_latin_azerbaijani(
            client=client,
            model=model,
            text=text,
            max_tokens=max_tokens * 2,
        )
        combined_lat.append(page_header + latin_text + "\n")
    ...
    out_cyr_path = out_txt_dir / f"{pdf_stem}.cyr.txt"
    out_lat_path = out_txt_dir / f"{pdf_stem}.lat.txt"

    out_cyr_path.write_text(
    "\n".join(combined_cyr).strip() + "\n",
    encoding="utf-8"
)

    out_lat_path.write_text(
        "\n".join(combined_lat).strip() + "\n",
        encoding="utf-8"
    )

    print(f"[DONE] {pdf_path.name}")
    print(f"  → Cyrillic: {out_cyr_path}")
    print(f"  → Latin:    {out_lat_path}")
